...
git config core.hooksPath .husky   # One-time: activate pre-commit hook (runs tidy:check)
...
- Source targets Java 8 (`maven.compiler.source`/`target` = `1.8`) — this is why the two tools below are pinned.
...
This repo uses gitflow (via `gitflow-maven-plugin`): branch from and target PRs at
`develop`, not `main`. Releases go through `./mvnw gitflow:release-start` /
`gitflow:release-finish`, merging into `main`.
